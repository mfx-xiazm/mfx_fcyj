...
@implementation FCHotelCommentCell

+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    static NSString *ID = @"GoodsCommentCell";
    FCHotelCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) {
...
-(UIButton *)moreLessBtn
{
    if (!_moreLessBtn) {
        ...
        _moreLessBtn.titleLabel.font = [UIFont systemFontOfSize:13];
...
#pragma mark - Setter
-(void)setCommentLayout:(FCHotelCommentLayout *)commentLayout
{
    _commentLayout = commentLayout;
    
    UIView * lastView;
    FCHotelComment *comment = _commentLayout.comment;
    ...
    _avatarView.image = HXGetImage(@"login_logo");
    ...
    _createTime.hxn_x = HX_SCREEN_WIDTH - kMomentMarginLeftRight - timeSize.width;
    ...
    _textContent.hxn_width = HX_SCREEN_WIDTH - kMomentMarginLeftRight * 2;
    ...
    //展开/收起按钮
    _moreLessBtn.hxn_x = kMomentMarginLeftRight;
    ...
    if (comment.shouldShowMoreButton) {
        ...
        if (comment.isOpening) {
            [_moreLessBtn setTitle:@"收起" forState:UIControlStateNormal];
        }else{
            [_moreLessBtn setTitle:@"展开" forState:UIControlStateNormal];
        }
    ...
    if (comment.photos.count != 0) {
        ...
        _picContainerView.hxn_x = kMomentMarginLeftRight;
