...
@interface FCAddFollowVC ()<UICollectionViewDelegate,UICollectionViewDataSource,ZLCollectionViewBaseFlowLayoutDelegate>
...
@implementation FCAddFollowVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self.navigationItem setTitle:@"添加跟进记录"];
    self.remark.placeholder = @"请输入跟进说明";
...
-(NSMutableArray *)showData
{
    if (_showData == nil) {
        ...
        [_showData addObject:HXGetImage(@"上传")];
    }
    return _showData;
}
- (ZLPhotoActionSheet *)getPas
{
    ...
    [actionSheet setSelectImageBlock:^(NSArray<UIImage *> * _Nonnull images, NSArray<PHAsset *> * _Nonnull assets, BOOL isOriginal) {
        ...
        if (self.showData.count != 9) {
            [self.showData addObject:HXGetImage(@"上传")];
            self.isSelect9 = NO;
        }else{
            self.isSelect9 = YES;
        }
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.picContent.image = self.showData[indexPath.item];
    return cell;
}
