...
@interface FCHouseRentVC ()<JXCategoryViewDelegate,UIScrollViewDelegate,UITextFieldDelegate>
...
-(NSArray *)childVCs
{
    if (_childVCs == nil) {
        ...
        for (int i=0;i<self.categoryView.titles.count;i++) {
            FCHouseRentChildVC *cvc0 = [FCHouseRentChildVC new];
...
-(void)setUpNavBar
{
    [self.navigationItem setTitle:@"应收/应支房租"];
}
-(void)setUpCategoryView
{
    _categoryView.backgroundColor = [UIColor whiteColor];
    _categoryView.titleLabelZoomEnabled = NO;
    _categoryView.titles = @[@"未收/未支", @"已收/已支"];
