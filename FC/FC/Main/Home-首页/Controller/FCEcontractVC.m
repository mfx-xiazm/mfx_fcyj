...
@interface FCEcontractVC ()<JXCategoryViewDelegate,UIScrollViewDelegate,UITextFieldDelegate>
...
-(NSArray *)childVCs
{
    if (_childVCs == nil) {
        ...
        for (int i=0;i<self.categoryView.titles.count;i++) {
            FCEcontractChildVC *cvc0 = [FCEcontractChildVC new];
...
-(void)setUpNavBar
{
    [self.navigationItem setTitle:@"电子合同"];
}
-(void)setUpCategoryView
{
    _categoryView.backgroundColor = [UIColor whiteColor];
    _categoryView.titleLabelZoomEnabled = NO;
    _categoryView.titles = @[@"租客合同", @"房东合同"];
