...
@interface FCOutRentNoteVC ()<JXCategoryViewDelegate,UIScrollViewDelegate,UITextFieldDelegate>
...
@implementation FCOutRentNoteVC
...
-(NSArray *)childVCs
{
    if (_childVCs == nil) {
        ...
        for (int i=0;i<self.categoryView.titles.count;i++) {
            FCOutRentNoteChildVC *cvc0 = [FCOutRentNoteChildVC new];
...
-(void)setUpNavBar
{
    [self.navigationItem setTitle:@"退房记录"];
}
